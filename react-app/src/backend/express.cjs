...
app.delete('/udito/:id', (req, res) => {
    ...
    const deleteFileLine = `${req.params?.id};${req.body.nev};${req.body.liter};${req.body["bubis-e"]}`
    try {
        // TODO - delete id line
...
app.use((req, res) => {
    res.status(404).sendFile(path.join(__dirname, 'error.html')) // __dirname --> OP lekérdezés
})
